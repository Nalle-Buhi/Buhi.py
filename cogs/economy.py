...
class Economy(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.startmoney = (
            50  # money amount which will be given on bank account creation
        )
    ...
    async def open_account(self, user_id):
        ...
        if results:
            ...
            cur.execute(
                f"INSERT INTO user_data VALUES (?, ?)", (user_id, self.startmoney)
            )
            con.commit()
        con.close()
    ...
    async def add_store(
        self, item_name, item_desc, item_price, in_buhinet, usable, item_function=""
    ):
    ...
    async def update_balance(self, user_id, change):
        ...
        cur = con.cursor()
        cur.execute(
            f"UPDATE user_data SET balance = balance + ? WHERE user_id=?",
            (change, user_id),
        )
        con.commit()
        con.close()
    ...
    async def update_buhicoin_balance(self, user_id, change):
        ...
        cur = con.cursor()
        cur.execute(
            f"SELECT item_name FROM inventory_data WHERE user_id=? AND item_name=?",
            (user_id, "Buhicoin"),
        )
        ...
        if inventory == None:
            cur.execute(
                ...
                (user_id, "Buhicoin", change),
            )
            con.commit()
        else:
            cur.execute(
                f"UPDATE inventory_data SET amount = amount + ? WHERE user_id=? AND item_name=?",
                (change, user_id, "Buhicoin"),
            )
            con.commit()
    ...
    async def update_inventory(self, user_id, item_name, change):
        ...
        if inventory == None:
            cur.execute(
                "INSERT INTO inventory_data values (?, ?, ?)",
                (user_id, item_name, change),
            )
            con.commit()
        else:
            cur.execute(
                f"UPDATE inventory_data SET amount = amount + ? WHERE user_id=? AND item_name=?",
                (change, user_id, item_name),
            )
            con.commit()
    ...
    async def get_usable(self, user_id):
        ...
        cur = con.cursor()
        cur.execute(
            "SELECT I.item_name, s.item_name, I.amount, s.item_desc, s.item_function FROM inventory_data I, store_data s WHERE I.user_id = ? AND usable = ? AND I.item_name = s.item_name",
            (user_id, 1),
        )
        return cur.fetchall()
        con.close()
    ...
    async def get_user_items(self, user_id):
        ...
        cur = con.cursor()
        cur.execute(
            "SELECT I.item_name, s.item_name, I.amount, s.item_desc, s.item_function FROM inventory_data I, store_data s WHERE I.user_id = ? AND I.item_name = s.item_name",
            (user_id,),
        )
        return cur.fetchall()
        con.close()
    ...
    async def get_user_item(self, user_id, item_name):
        ...
        cur = con.cursor()
        cur.execute(
            "SELECT * FROM inventory_data WHERE user_id=? AND item_name=?",
            (user_id, item_name),
        )
        store_data = cur.fetchone()
        return store_data
    ...
    async def buhicoin_price(self, action, price=None):
        ...
        if action == "check":
            cur.execute(
                f"SELECT item_price FROM store_data WHERE item_name=?", ("Buhicoin",)
            )
            price = cur.fetchone()
        elif action == "update":
            cur.execute(
                f"UPDATE store_data SET item_price = ? WHERE item_name=?",
                (price, "Buhicoin"),
            )
            con.commit()
    ...
    async def buhiminer_checker(self):
        ...
        cur = con.cursor()
        cur.execute(
            "SELECT * FROM inventory_data WHERE item_name=? AND amount > ?",
            ("Buhicoin miner", 0),
        )  # get all rows which have miner and its value is over 1
    ...
    async def tili(self, ctx):
        ...
        em = await embed_builder(
            ctx,
            title="Tilisi tiedot",
            description=f"Tililläsi on {balance:.2f}ඞ!",
            image=image_url,
        )
    ...
    async def kauppa(self, ctx):
        ...
        for item in store_data:
            fields.append(
                [f"{item[0]}\nHinta: {item[2]}ඞ", item[1], False]
            )  # append items to list where index 0 = item_name, 1 = item_desc, 2 = item_price
            shop_list.append(
                [f"{item[0]}: {item[2]}ඞ", item[1], item[0]]
            )  # append items to list where index 0 = item_id, 1 = item_name, 2 = item_desc, 3 = item_price
        view = uitools.ShopView(
            ctx,
            "Valitse tavarat jotka haluat ostaa",
            1,
            len(shop_list),
            shop_list,
            True,
        ...
        em = await embed_builder(
            ctx,
            "Buhin kauppa",
            "valikoima saattaa vaihdella, ostaminen: select menun kautta",
            fields=fields,
            image="https://raw.githubusercontent.com/Nalle-Buhi/Buhi.py/main/images/buhit%C3%B6is.png",
        )
        ...
        for i in view.values:
            ...
            em = await embed_builder(
                ctx,
                "Kuinka monta haluat ostaa",
                f"{i}: {price}ඞ",
                fields=[["Tililläsi on rahaa", f"{user_balance}ඞ", True]],
            )
            await sent_em.edit(embed=em, view=None)
            amt = await self.bot.wait_for(
            ...
            if receipt_checker == True:
                ...
                em = await embed_builder(
                    ctx, "Kuitti", "Ostit juuri nämä buhin kaupasta", fields=receipt
                )
            else:
                pass
        await sent_em.edit(embed=em)
    ...
    async def käytä(self, ctx):
        ...
        if data:
            ...
            view = uitools.SelectFromList(
                ctx, "Valitse tavarat", 1, 1, user_items, True, 60
            )
            ...
            em = await embed_builder(
                ctx,
                "Kuinka monta haluat käyttää",
                f"{user_items[0][1]} kappaletta {item_name}",
            )
            await sent_em.edit(embed=em, view=None)
            ...
            if int(amt) > 5 or int(amt) <= 0:
                ...
                if user_item:
                    if user_item[2] >= int(amt):
                        ...
                        view = uitools.Confirm(ctx)
                        await ctx.send(
                            f"Oletko varma että haluat käyttää {amt} kappaletta {item_name}",
                            view=view,
                        )
                        await view.wait()
                        if view.value == True:
                            await self.update_inventory(
                                ctx.author.id, item_name, -int(amt)
                            )
                            try:
        ...
        if int(amt) <= 0:
            ...
            if user_item:
                if user_item[2] >= int(amt):
                    ...
                    view = uitools.Confirm(ctx)
                    await ctx.send(
                        f"Oletko varma että haluat käyttää {amt} kappalettä {item_from_shop[1]}",
                        view=view,
                    )
                    await view.wait()
                    if view.value == True:
                        ...
                        await self.update_balance(
                            ctx.author.id, (int(amt) * item_from_shop[3] * 0.25)
                        )
    ...
    async def top5(self, ctx):
        ...
        cur = con.cursor()
        cur.execute(
            "SELECT balance, user_id FROM user_data ORDER BY balance DESC LIMIT 5"
        )
        ...
        em = await embed_builder(
            ctx,
            "Top 5 serverin rikkaimmat",
            " ",
            fields=fields,
            image="https://raw.githubusercontent.com/Nalle-Buhi/Buhi.py/main/images/avatar.png",
        )
        await ctx.send(embed=em)
    ...
    async def buhicoin(self, ctx):
        ...
        if result:
            ...
            em = await embed_builder(
                ctx,
                "Buhicoin market:",
                f"Buhicoinin hinta on: {price}ඞ",
                image="https://raw.githubusercontent.com/Nalle-Buhi/Buhi.py/main/images/buh_coin_better.png",
            )
            sent_em = await ctx.send(embed=em, view=view)
            await view.wait()
            if view.value == "buy":
                ...
                em = await embed_builder(
                    ...
                    "Kuinka monta haluat ostaa",
                    f"{item_name}: {price}ඞ",
                    fields=[["Tililläsi on rahaa", f"{user_balance:.2f}ඞ", True]],
                )
                await sent_em.edit(embed=em, view=None)
                amt = await self.bot.wait_for(
                    "message", check=lambda message: message.author == ctx.author
                ...
                if amount > 0 and user_balance >= total_price:
                    ...
                    em = await embed_builder(
                        ctx,
                        "Kuitti",
                        "Ostit juuri buhkolikkeja",
                        fields=[
                            [
                                f"{item_name}: {amt.content} Kpl",
                                f"{total_price:.2f}ඞ",
                                False,
                            ]
                        ],
                ...
                em = await embed_builder(
                    ctx,
                    "Kuinka monta haluat myydä",
                    f"{item_name}: {price}ඞ",
                    fields=[["Sinulla on ", f"{user_coins[2]:.2f} buhicoinia", True]],
                )
                await sent_em.edit(embed=em, view=None)
                amt = await self.bot.wait_for(
                    "message", check=lambda message: message.author == ctx.author
                ...
                if amount <= 0:
                    ...
                    em = await embed_builder(
                        ctx,
                        "Kuitti",
                        "Myit juuri buhkolikkeja",
                        fields=[
                            [
                                f"{item_name}: {amt.content} Kpl",
                                f"Sait {(price * amount):.2f}ඞ",
                                False,
                            ]
                        ],
                    )
        ...
        if self.kerjää.is_on_cooldown(ctx):
            ...
            if random.randint(0, 100) == 100:
                ...
                em = await embed_builder(
                    ctx,
                    "Kerjäsit rahaa",
                    f"Ja sait rahaa {money_got}ඞ",
                    image="https://raw.githubusercontent.com/Nalle-Buhi/Buhi.py/main/images/ei_raha_apu.jpg",
                )
                await ctx.send(embed=em)
    ...
    async def pizza(self, ctx):
        if self.pizza.is_on_cooldown(ctx):
            ...
            em = await embed_builder(
                ctx,
                "Vie pizza oikeaan osoitteeseen",
                f"Oikea osoite {delivery_address}",
            )
            sent_em = await ctx.send(embed=em, view=view)
            await view.wait()
            print(view.value)
            if str(view.value) in delivery_address:
                ...
                new_em = await embed_builder(
                    ctx,
                    "Toimitit pizzan onnistuneesti",
                    f"Sait pizzan toimituksesta {earned}ඞ",
                    image="https://raw.githubusercontent.com/Nalle-Buhi/Buhi.py/main/images/pizza_apu.jpg",
                )
                ...
                new_em = await embed_builder(
                    ctx,
                    "Vitun tunari et toimittanut pitsaa",
                    f"Jouduit maksamaan korvauksia {reimbursement}ඞ",
                    image="https://raw.githubusercontent.com/Nalle-Buhi/Buhi.py/main/images/suruviha_apu.jpg",
                )
    ...
    async def pizzeria(self, ctx):
        if self.pizzeria.is_on_cooldown(ctx):
            ...
            view = uitools.SelectFromList(
                ctx, "Valitse pitsan täytteet oikein", 1, len(order), toppings, True, 10
            )
            em = await embed_builder(
                ctx,
                "Pizzatilaus, tee pizza asiakkaan tilauksen mukaan",
                f"Tilaus: {', '.join(order)}",
                image="https://raw.githubusercontent.com/Nalle-Buhi/Buhi.py/main/images/buhi_pizzeria.png",
            )
            sent_em = await ctx.send(embed=em, view=view)
            await view.wait()
            if view.values == None:  # view.values is none on timeout error
                ...
                new_em = await embed_builder(
                    ctx,
                    "Et ehtinyt tekemään pizzaa",
                    f"Maksoit asiakkaalle korvauksia myöhästyneestä pizzasta {reimbursement}ඞ",
                    image="https://raw.githubusercontent.com/Nalle-Buhi/Buhi.py/main/images/suruviha_apu.jpg",
                )
                await self.update_balance(ctx.author.id, -reimbursement)
            elif set(order) == set(
                view.values
            ):  # checks if order list and selected values are the same, selected order doesn't matter
                earned = random.randint(15, 30)
                new_em = await embed_builder(
                    ctx,
                    "Teit pizzan asiakkaan tilauksen mukaan!",
                    f"Sait pizzan teosta {earned}ඞ",
                    image="https://raw.githubusercontent.com/Nalle-Buhi/Buhi.py/main/images/pizza_apu.jpg",
                )
                await self.update_balance(ctx.author.id, earned)
                ...
                if random.randint(0, 50) == 0:
                    ...
                    new_em = await embed_builder(
                        ctx,
                        "Asiakas oli allerginen tekemällesi pitsalle ja melkein kuoli saatana",
                        f"Maksoit sairaala kuluja {reimbursement}ඞ",
                        image="https://raw.githubusercontent.com/Nalle-Buhi/Buhi.py/main/images/suruviha_apu.jpg",
                    )
                else:
                    ...
                    new_em = await embed_builder(
                        ctx,
                        "Teit väärän pizzan ja asiakas oli vihainen",
                        f"Maksoit korvauksia {reimbursement}ඞ",
                        image="https://raw.githubusercontent.com/Nalle-Buhi/Buhi.py/main/images/suruviha_apu.jpg",
                    )
                await self.update_balance(ctx.author.id, -reimbursement)
            else:
    ...
    async def kassa(self, ctx):
        if self.kassa.is_on_cooldown(ctx):
            ...
            em = await embed_builder(
                ctx,
                "Olet töissä kaupan kassalla, laske vaihtorahat",
                "Vastaukset kahden desimaalin tarkkuudella, paina confirm kun olet valmis",
            )
            view = uitools.Confirm(ctx)
            sent_em = await ctx.send(embed=em, view=view)
            await view.wait()
            if view.value == True:
                ...
                given_change = round(
                    random.uniform(item_prices, (item_prices + 100)), 2
                )
                em = await embed_builder(
                    ctx,
                    "Laske vaihtorahat",
                    f"Tavaroiden arvo {item_prices}ඞ. Asiakas antoi {given_change}ඞ",
                )
                await sent_em.edit(embed=em, view=None)
                ...
                if float(message.content) == round(given_change - item_prices, 2):
                    ...
                    em = await embed_builder(
                        ctx,
                        "Laskit vaihtorahat oikein",
                        f"Buhi maksoi sinulle {earned}ඞ tuuraamisesta",
                    )
                else:
                    reimbursement = random.randint(15, 30)
                    await self.update_balance(ctx.author.id, -reimbursement)
                    em = await embed_builder(
                        ctx,
                        "Vitun tunari",
                        f"Buhin kauppa menetti asiakkaan, sinä menetit {reimbursement}ඞ ja kaupanpäälle sait buhilta turpaan",
                    )
                await sent_em.edit(embed=em)
            else:
    ...
    async def flip(self, ctx):
        balance = await self.check_balance(ctx.author.id)
        await ctx.send(
            f"paljonko haluat betata? sinulla on käytössä: {balance}ඞ, voittokerroin on 1.9x ja minimi panos on 10ඞ"
        )
        message = await self.bot.wait_for(
            "message", check=lambda message: message.author == ctx.author
        )
        ...
        if bet > balance or bet < 10:
            await ctx.send(
                "Sinulla ei ole noin paljon rahaa, tai minimipanos liian pieni"
            )
        else:
            ...
            em = await embed_builder(
                ctx,
                "Kumman valitset?",
                "<:kassbuhi:899778843148165191>: Kassbuhi vai <:mulkkuankka:784516182639247360>: Mulkkuankka",
            )
            ...
            r, u = await self.bot.wait_for(
                "reaction_add",
                timeout=30,
                check=lambda r, u: r.emoji.id in emojis and u == ctx.author,
            )
            if flip == r.emoji.id:
                await self.update_balance(ctx.author.id, (bet * 0.9))
                em = await embed_builder(
                    ctx, "voitit", "onneksi onlkoo", image=self.bot.get_emoji(flip).url
                )
                ...
                em = await embed_builder(
                    ctx,
                    "hävisit pelin",
                    "onneksi voit pelata uudellen",
                    image=self.bot.get_emoji(flip).url,
                )
                await choose.edit(embed=em)
    ...
    async def add_item(self, ctx, *, args):
        ...
        item_name, item_desc, item_price, in_buhinet, usable, func = map(
            lambda x: x.strip(), args.split("|")
        )
        view = uitools.Confirm(ctx)
        await ctx.send(
            f"Lisätäänkö kauppaan: {item_name}, {item_desc}, {item_price}ඞ, buhinetissä?: {in_buhinet}, käytettävissä?: {usable} ```python\n{func}```?",
            view=view,
        )
        await view.wait()
        if view.value == True:
            adder = await self.add_store(
                item_name, item_desc, item_price, in_buhinet, usable, func
            )
            if adder == False:
                await ctx.send(
                    "Kaupassa on jo kyseinen itemi, koita poistaa itemi tai päivittää sitä"
                )
            else:
                await ctx.send(
                    f"Kauppaan lisätty: {item_name}, {item_desc}, {item_price}ඞ, buhinetissä?: {in_buhinet}, käytettävissä?: {usable} ```python\n{func}```"
                )
    ...
    async def delete_item(self, ctx, *, item_name):
        view = uitools.Confirm(ctx)
        await ctx.send(
            f"oletko varma että haluat poistaa itemin id:llä {item_name}", view=view
        )
        await view.wait()
        if view.value == True:
    ...
    async def update_item(self, ctx, *, args):
        ...
        item_name, new_name, item_desc, item_price, in_buhinet, usable, func = map(
            lambda x: x.strip(), args.split("|")
        )
        view = uitools.Confirm(ctx)
        await ctx.send(
            f"Päivitetäänkö itemi?: {item_name}, {item_desc}, {item_price}ඞ, buhinetissä?: {in_buhinet}, käytettävissä?: {usable} ```python\n{func}```?",
            view=view,
        )
        await view.wait()
        if view.value == True:
            updater = await self.update_store(
                item_name, new_name, item_desc, item_price, in_buhinet, usable, func
            )
            if updater == False:
                await ctx.send("itemiä ei olemassa")
            else:
                await ctx.send(
                    f"Kauppa päivitetty: {item_name}, {item_desc}, {item_price}ඞ, buhinetissä?: {in_buhinet}, käytettävissä?: {usable} ```python\n{func}```"
                )
