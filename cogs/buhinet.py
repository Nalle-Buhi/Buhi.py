...
class Buhinet(commands.Cog):
    ...
    async def get_buhinet_item(self, item_name):
        ...
        cur = con.cursor()
        cur.execute(
            "SELECT * FROM store_data WHERE in_buhinet = ? AND item_name=?",
            (1, item_name),
        )
        store_data = cur.fetchone()
        return store_data
    ...
    async def buhinet(self, ctx):
        ...
        if result:
            ...
            em = await embed_builder(
                ctx,
                "Tervetuloa buhinettiin",
                "buhinet on buhi botissa oleva peittoverkko, jota voidaan käyttää vain tietyllä ohjelmistolla, kokoonpanoilla tai valtuutuksilla ja joka käyttää usein ainutlaatuista mukautettua viestintäprotokollaa",
                image="https://raw.githubusercontent.com/Nalle-Buhi/Buhi.py/main/images/buhnet.png",
            )
            ...
            if view.value == "buy":
                ...
                for i in view.values:
                    ...
                    em = await embed_builder(
                        ...
                        f"{i}: {price}ඞ",
                        fields=[["Sinulla on buhicoineja", f"{user_balance} BC", True]],
                    )
                    ...
                    if (int(amt.content) * price) <= user_balance:
                        ...
                        await self.update_buhicoin_balance(
                            ctx.author.id, -(int(amt.content) * price)
                        )
                        await self.update_inventory(ctx.author.id, i, int(amt.content))
                        total_price = price * int(amt.content)
                        receipt.append(
                            [f"{i}: {amt.content} Kpl", f"{total_price}ඞ", False]
                        )
                        em = await embed_builder(
                            ctx,
                            "Buhinet ostokset tehty",
                            "Ostit juuri nämä",
                            fields=receipt,
                        )
                    else:
                        await ctx.send(
                            f"Sinulla ei ole tarpeeksi buhicoineja ostaa {i}"
                        )
